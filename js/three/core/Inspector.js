...
function scanWindow() {
  ...
  divRescan.onclick = ThreeInspector.start.bind(this, threeObject);
  ...
  addInspectChild(
    threeObject.camera,
    ThreeInspectorWidget.contents,
    `${(threeObject.el?.id || 'CAMERA_' + threeObject.camera.id).toUpperCase()} Camera`
  );

  // Register Materials
  threeObject.scene.traverse((obj) => {
    if (obj.material) {
      materialsToInspect.push(obj.material);
      materialNamesToInspect.push(obj.name);
    }
  });

  // Register Name

  var sceneName = `${(threeObject.el?.id || 'SCENE_' + threeObject.scene.id).toUpperCase()} Scene`;

  aScene = threeObject.scene;
  ...
  expander.innerHTML = "+ <b>" + sceneName + "</b>";
  expander.onclick = expandScene(ul, aScene, sceneName);
  ...
  materialsToInspect.forEach(function (material, i) {
    ...
    addInspectChild(material, ThreeInspectorWidget.contents, "material_" + i);
...
function Widget(title, id, targetDom, method) {
  ...
  function onMouseMove(e) {
    ...
    if (Math.abs(e.clientY - offsetY) < snapDistance) {
      ...
      Math.abs(e.clientY + offsetFromBottom - window.innerHeight) < snapDistance
    ) {
...
ThreeInspector.start = function (three) {
  // Destory previous copy of ThreeInspector.
  if (window.ThreeInspector) {
    
    window.ThreeInspector.destroy();
  ...
  if (ThreeInspector.newWindow) {
    ...
    ThreeInspectorWidget.setSize(.25 * innerWidth, .5 * innerHeight);
...
ThreeInspector.destroy = function () {

  materialsToInspect      = [];
  materialNamesToInspect  = [];
  ...
  if (ThreeInspector.sidePane) {
    ThreeInspector.sidePane.close();
  }
